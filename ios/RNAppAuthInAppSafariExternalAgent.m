...
@interface RNAppAuthInAppSafariExternalAgent ()<SFSafariViewControllerDelegate>
@end
#else
@interface RNAppAuthLogoutExernalAgent ()<SFSafariViewControllerDelegate>
@end
#endif

@implementation RNAppAuthInAppSafariExternalAgent {
    UIViewController *_presentingViewController;

    BOOL _externalUserAgentFlowInProgress;
    BOOL _executesLogout;
...
- (BOOL)presentExternalUserAgentRequest:(id<OIDExternalUserAgentRequest>)request
    ...
    if (@available(iOS 9.0, *)) {
        if (!openedUserAgent && _presentingViewController) {
            ...
            _safariVC = safariVC;
            _presentingViewController.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
            _presentingViewController.modalPresentationStyle = UIModalPresentationFullScreen;
...
- (void)safariViewController:(SFSafariViewController *)controller initialLoadDidRedirectToURL:(NSURL *)URL {
    if ([[URL absoluteString] containsString:@"logout"]) {
        _executesLogout = YES;
    }
    if (_executesLogout == YES && [[URL absoluteString] containsString:@"login"]) {
        _executesLogout = NO;
        self.onLogoutSignInRedirection();
        self.onLogoutSignInRedirection = nil;
    }
}
